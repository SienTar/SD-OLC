...
{
  objecter->init();
  //Connect Redis and get some data
  char ip_temp[16];
  ...
  for(int i=0;i<NUM_OSD;i++){
        ...
        queue_map.push_back(osd_load_base[i]);
        accumulate_queue_map.push_back(osd_load_base[i]);
        dout(0)<<"mydebug: osd->queue_map["<<i<<"]="<<queue_map[i]<<dendl;
        dout(0)<<"mydebug: osd->accumulate_queue_map["<<i<<"]="<<accumulate_queue_map[i]<<dendl;
  }
  for(int i=0;i<NUM_OSD;i++){
